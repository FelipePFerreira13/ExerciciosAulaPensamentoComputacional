...
def show_predios():
    ...
    def deletar_predio():
        try:
            ...
            predio = next((p for p in Db_Tools.Puxar_Predios() if p.id == predio_id), None)
            if predio:
                session = Db_Tools.__dict__.get('session', None)
                if session:
                    session.delete(predio)
                    session.commit()
                else:
                    # fallback para session global
                    from database.database_tools import session as global_session
                    global_session.delete(predio)
